...
public class BiomeColormap implements ColormaticResolver {
	...
	private int computeDefaultColor(ColormapProperties props) {
		...
		case GRID:
			try {
				int x = props.getColumn(BiomeKeys.PLAINS,
						Colormatic.CURRENTBIOMES).column;
	...
	/**
	 * Returns a color given by the custom colormap for the given biome temperature
	 * and humidity.
	 */
	 ...
	 * account the client's biome blend options If either `world` or `pos` is null,
	 * this returns the colormap's default color.
	 */
