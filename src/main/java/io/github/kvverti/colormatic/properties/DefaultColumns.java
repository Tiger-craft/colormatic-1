...
public final class DefaultColumns {
	...
	/**
	 * The 1.17 mapping of biomes to columns. Uses the 1.17 column equivalents for
	 * custom biomes and doubles up columns for new vanilla biomes.
	 */
	private static final Map<Identifier, ColormapProperties.ColumnBounds> legacyColumns = createLegacyColumnBounds();

	/**
	 * A stable mapping of biomes to columns. Never uses biome raw IDs to determine
	 * column.
	 */
	private static final Map<Identifier, ColormapProperties.ColumnBounds> stableColumns = createStableColumnBounds();

	private static final int VANILLA_BIOME_COUNT = Colormatic.CURRENTBIOMES.size();
	...
	/**
	 * Returns columns based on vanilla biomes' raw IDs and datapack biomes' nearest
	 * temperature neighbors. The default for colormaps in the Colormatic namespace.
	 */
	 ...
	 */
	public static ColormapProperties.ColumnBounds getOptifineBounds(RegistryKey<Biome> biomeKey,
			Map<Identifier, Integer> biomes) {
		var bounds = currentColumns.get(biomeKey.getValue());
		if (bounds == null) {
			// Optifine computes grid colors using the raw ID
			int rawID = biomes.get(biomeKey).intValue();
	...
	/**
	 * The 1.17 vanilla biome column bounds, with new vanilla biomes assigned to an
	 * approximate. Custom biomes are offset so that the first custom biome takes
	 * column 176.
	 */
	public static ColormapProperties.ColumnBounds getLegacyBounds(RegistryKey<Biome> biomeKey,
			Map<Identifier, Integer> biomes, boolean optifine) {
	...
	/**
	 * Returns the stable column bounds for the given biome. Vanilla biome columns
	 * are guaranteed to be stable across versions, and custom biomes are always
	 * approximated with a vanilla biome.
	 */
	...
	private static Identifier approximateToVanilla(RegistryKey<Biome> biomeKey) {
		...
		if (id == null) {
			// this exception tends to trigger a crash in the crash report generator due to
			// it
	...
	/**
	 * Called when the dynamic registry manager is changed to re-generate default
	 * column bounds. This keeps column bounds in sync with the available biome
	 * list.
	 *
	...
	/**
	 * Given a custom biome, finds the vanilla biome closest in temperature and
	 * humidity to the given biome and returns its bounds.
	 *
	...
	private static Map<Identifier, ColormapProperties.ColumnBounds> createCurrentColumnBounds() {
		...
		Colormatic.CURRENTBIOMES.forEach((id, rawId) -> {
			map.put(id, new ColormapProperties.ColumnBounds(rawId.intValue(), 1));
		});
